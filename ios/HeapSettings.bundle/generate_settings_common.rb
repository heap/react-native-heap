...
def perform_substitution(dir, json)
  ...
  set_values_simple('EnableDebugLogging', 'bool', dir, json, 'debug', false)
  
  set_values('AppId', 'string', dir, json) { |s, d|
...
def set_values_simple(key, type, dir, json, json_key, valueIfNil)
  set_values(key, type, dir, json) { |s, d| get_property(json_key, valueIfNil, s, d) }
end
...
def set_values(key, type, dir, json, &block)
  ...
  set_value(":Heap#{key}Dev", type, block.call(dev, default), output_path)
  set_value(":Heap#{key}Prod", type, block.call(prod, default), output_path)
end

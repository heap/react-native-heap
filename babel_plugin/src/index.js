...
const hasTouchableMixin = (path) => {
  ...
  return !!path.container.find((node, i) => {
    if (node.type === 'ObjectProperty' && node.key.name === 'mixins' && node.value.type === 'ArrayExpression') {
...
function transform (babel) {
  return {
    visitor: {
      ObjectProperty(path) {
        if (path.node.key.name === 'touchableHandlePress' || path.node.key.name === 'touchableHandleLongPress') {
          const parent = path.findParent(path => path.isCallExpression() && path.node.callee.name === 'createReactClass');
          if (parent) {
            if (!hasTouchableMixin(path)) {
              return;
            }
            ...
            const callOriginalFunctionExpression = t.memberExpression(originalFunctionExpression, t.identifier('call'));
            const calledFunction = t.callExpression(callOriginalFunctionExpression, [t.identifier('this'), t.identifier('e')]);

            // Create the expression for calling Heap autotrack.
            ...
            const autotrackExpression = t.callExpression(
              ...
              [t.stringLiteral(path.node.key.name), t.identifier('this'), t.identifier('e')]);
            ...
            const replacementFunc = t.arrowFunctionExpression([t.identifier('e')], functionBody);
